...
def create_tables():
    with conn.cursor() as cur:
        # Sales Table
        cur.execute('''
            CREATE TABLE IF NOT EXISTS web_sales_bills (
                id SERIAL PRIMARY KEY, 
                inv_no VARCHAR(50) NOT NULL UNIQUE, 
        ...
        cur.execute('''
            CREATE TABLE IF NOT EXISTS web_business_profile (
                id INTEGER PRIMARY KEY, firm_name VARCHAR(255), address TEXT, contact VARCHAR(50), gstin VARCHAR(50)
            )
        ''')
        cur.execute('''
            CREATE TABLE IF NOT EXISTS web_proforma_invoices (
                id SERIAL PRIMARY KEY, inv_no VARCHAR(50), date DATE, time TIME, party_name VARCHAR(255),
                address TEXT, mobile VARCHAR(50), gstin VARCHAR(50), subtotal DECIMAL(15,2),
                discount_total DECIMAL(15,2), tax_total DECIMAL(15,2), grand_total DECIMAL(15,2), notes TEXT
            )
        ''')
        cur.execute('''
            CREATE TABLE IF NOT EXISTS web_proforma_items (
...
st.markdown("""
    ...
    .summary-box { background-color: #1A1A24; padding: 15px; border-radius: 10px; border: 1px solid #333; }
...
if "proforma_items" not in st.session_state:
    st.session_state.proforma_items = pd.DataFrame(
        columns=["Item Name", "Qty", "Price (Incl. GST)", "Disc %", "Tax %"],
        data=[["", 1, 0.0, 0, 0] for _ in range(5)]
    )
...
# ==========================================
    ...
    st.info("Purchase Register module is active as per previous code integrations.")
...
# ==========================================
    ...
    if st.session_state.proforma_tab == "entry":
        ...
        edited_df = st.data_editor(
            st.session_state.proforma_items, num_rows="dynamic", use_container_width=True,
            column_config={
                "Qty": st.column_config.NumberColumn(min_value=1),
                "Price (Incl. GST)": st.column_config.NumberColumn(format="₹%.2f"),
                "Disc %": st.column_config.NumberColumn(min_value=0, max_value=100),
                "Tax %": st.column_config.NumberColumn(min_value=0, max_value=100),
            }
        )
        ...
        for index, row in edited_df.iterrows():
            if row["Item Name"].strip() != "":
                qty = float(row["Qty"] or 0); price_incl = float(row["Price (Incl. GST)"] or 0)
                disc_perc = float(row["Disc %"] or 0); tax_rate = float(row["Tax %"] or 0)
        ...
        s1.metric("Sub Total", f"₹{total_taxable:,.2f}"); s2.metric("Discount", f"₹{total_disc:,.2f}")
        s3.metric("Total Tax", f"₹{total_tax:,.2f}"); s4.markdown(f'<div class="grand-total">Grand Total: ₹{grand_total:,.2f}</div>', unsafe_allow_html=True)
    ...
    # --- TAB 3: BUSINESS PROFILE ---
        ...
        with st.form("profile_form"):
            ...
            if st.form_submit_button("💾 Save Profile"):
                with conn.cursor() as cur:
                    cur.execute("UPDATE web_business_profile SET firm_name=%s, address=%s, contact=%s, gstin=%s WHERE id=1", (new_firm, new_addr, new_cont, new_gst))
                st.success("Profile Updated!"); st.rerun()

# ==========================================
# 💰 SALES REGISTER (Fully Integrated)
# ==========================================
    ...
    if st.session_state.sales_tab == "entry":
        with st.form("sales_form", clear_on_submit=True):
            ...
            submit = st.form_submit_button("💾 SAVE SALES BILL", use_container_width=True)

            if submit:
                if not inv_no or not customer or total <= 0:
                    st.error("⚠️ Invoice No, Customer Name, and Total Amount are required!")
                else:
                    ...
                    try:
                        with conn.cursor() as cur:
                            cur.execute("""
                                INSERT INTO web_sales_bills 
                                (inv_no, inv_date, customer_name, gstin, items, qty, taxable, cgst, sgst, igst, total, remarks, file_name, file_data)
                        ...
                        st.error(f"⚠️ Invoice Number '{inv_no}' already exists in database!")
                    except Exception as e:
                        st.error(f"Database Error: {e}")

    # --- TAB 2: VIEW SALES REGISTER ---
        ...
        try:
            # Query Database
            query = """
                SELECT id, inv_no, inv_date, customer_name, gstin, items, qty, taxable, cgst, sgst, igst, total, remarks, file_name 
                FROM web_sales_bills 
                WHERE inv_date BETWEEN %s AND %s
            """
            ...
            if df.empty:
                ...
                m1.metric("Visible Sales", len(df))
                m2.metric("Total Qty", int(df['qty'].sum()))
                m3.metric("Total Taxable", f"₹{df['taxable'].sum():,.2f}")
                ...
                st.divider()
                st.write("**Invoice Actions (Generate PDF / Download Attachment)**")
                selected_id = st.selectbox("Select Invoice ID to Action:", df['id'].tolist())
                ...
                with col_action1:
                    if st.button("🖨️ Generate Web Invoice"):
                        with conn.cursor() as cur:
                            ...
                            html_content = f"""
                            <div style="background:white; color:black; padding:30px; font-family:sans-serif; border:1px solid #ddd; max-width: 800px; margin: auto;">
                                ...
                                <p style="text-align:center;">Commercial Market, Main Road<br><b>GSTIN:</b> 20XXXXXXXXXX1Z5</p>
                                <hr>
                                <div style="display:flex; justify-content:space-between; margin-bottom: 20px;">
                                    <div><b>Invoice No:</b> {inv[1]}<br><b>Billed To:</b> {inv[3]}<br><b>Customer GSTIN:</b> {inv[4]}</div>
                                    <div style="text-align:right;"><b>Date:</b> {inv[2]}</div>
                                </div>
                                <table style="width:100%; border-collapse: collapse; margin-bottom: 20px;">
                                    <tr style="background:#e6e6e6; font-weight:bold;">
                                        <th style="padding:10px; border:1px solid #000; text-align:left;">Item Description</th>
                                        <th style="padding:10px; border:1px solid #000;">Qty</th>
                                        <th style="padding:10px; border:1px solid #000;">Taxable (Rs)</th>
                                        <th style="padding:10px; border:1px solid #000;">CGST (Rs)</th>
                                        <th style="padding:10px; border:1px solid #000;">SGST (Rs)</th>
                                        <th style="padding:10px; border:1px solid #000;">IGST (Rs)</th>
                                    </tr>
                                    <tr>
                                        <td style="padding:10px; border:1px solid #000;">{inv[5]}</td>
                                        <td style="padding:10px; border:1px solid #000; text-align:center;">{inv[6]}</td>
                                        <td style="padding:10px; border:1px solid #000; text-align:right;">{inv[7]:.2f}</td>
                                        <td style="padding:10px; border:1px solid #000; text-align:right;">{inv[8]:.2f}</td>
                                        <td style="padding:10px; border:1px solid #000; text-align:right;">{inv[9]:.2f}</td>
                                        <td style="padding:10px; border:1px solid #000; text-align:right;">{inv[10]:.2f}</td>
                                    </tr>
                                </table>
                                <h2 style="text-align:right;">Grand Total: Rs. {inv[11]:.2f}</h2>
                                <p><b>Remarks:</b> {inv[12]}</p>
                                <br><br>
                                <p style="text-align:right;"><b>Authorized Signatory</b></p>
                                <button onclick="window.print()" style="padding: 10px 20px; background: #00E5FF; color: black; font-weight:bold; border: none; cursor: pointer; margin-top:20px;">Print / Save PDF</button>
                            </div>
                            """
                ...
                with col_action2:
                    with conn.cursor() as cur:
                        ...
                        if file_info and file_info[1]:
                            st.download_button(
                                label="📂 Download Attached Bill",
                                data=file_info[1],
                                file_name=file_info[0],
                                mime="application/octet-stream"
                            )
                        else:
                            st.button("📂 No File Attached", disabled=True)

        except Exception as e:
            st.error(f"Error loading sales data: {e}")

st.markdown('<div class="tagline">SALES & ERP MODULE - DEVELOPED BY ER. MANJIT BALMIKI</div>', unsafe_allow_html=True)
